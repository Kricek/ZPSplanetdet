...
sampler.reset()
sampler.run_mcmc(pos_new, 6600, progress=True)
...
mask = filtr2
results_1 = flat_samples[mask, 0:4]
results_2 = flat_samples[~mask, 0:4]
results_3 = flat_samples[:, 0:4]

sum1 = np.sum(mask)
sum2 = np.sum(~mask)

figure1 = corner.corner(results_1, labels=labels, quantiles=[0.16, 0.5, 0.84],              show_titles=True, title_kwargs={"fontsize": 12}, color='b')
corner.corner(results_2, labels=labels, quantiles=[0.16, 0.5, 0.84], fig=figure1, color='r', weights=(sum1/sum2)*np.ones(sum2))

figure2 = corner.corner(results_3, labels=labels, quantiles=[0.16, 0.5, 0.84],     show_titles=True, title_kwargs={"fontsize": 12}, color='k')
